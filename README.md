...
- Passively captures all HTTP traffic using Firefox's `webRequest` API. Response bodies are captured via `filterResponseData` (primary) with two fallback mechanisms: cache-based re-fetch for GET requests, and page-level XHR/`fetch` hooking via `<script>` tag injection for POST responses where `filterResponseData` produces no data. For mutating requests (POST/PUT/DELETE/PATCH), the fetch hook inlines body reading into the promise chain to prevent navigation races; GET/HEAD uses a non-blocking detached read. Content script injection is scoped to monitored tabs only. Captured data is stored in in-memory ring buffers -- 500 requests per tab, up to 20 tabs.
